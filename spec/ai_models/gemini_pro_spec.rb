...
RSpec.describe 'Using Gemini Pro model' do
  ...
  describe 'analyze feature' do
    describe '#analyze_sentence' do
      it 'return the sentiment of the sentence' do
        expect(sentiment.analyze_sentence('Delicious food')).to eq({sentence: 'Delicious food', sentiment: 'positive'})
        expect(sentiment.analyze_sentence('Too noisy!!!')).to eq({sentence: 'Too noisy!!!', sentiment: 'negative'})
        expect(sentiment.analyze_sentence("I really don't know how to feel about Pokemon")).to eq({sentence: "I really don't know how to feel about Pokemon", sentiment: 'neutral'})
      end
    end
